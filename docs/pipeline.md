...
### 6.5. Подтверждение (`internal/mcpserver`, FR-44–46)

Мутирующий вызов спрашивает человека — **после** всего, что может отказать без него: аргументы провалидированы, запрос собран, egress проверен. Значит, приглашение показывается только для вызова, который иначе бы состоялся, а «да» не делает ничего, кроме отправки.

На протоколе `2026-07-28` сервер не может открыть elicitation, пока обслуживает запрос: вместо этого handler возвращает multi-round-trip input request (SEP-2322), клиент его выполняет и вызывает инструмент снова — уже с ответом. Ждать нечего, таймаута нет: клиент, который не вернулся, просто не сделал вызов.

Режимы: `always` (по умолчанию), `client-capability`, `never`. Клиент, не заявивший elicitation, при `always` получает отказ до сети (`approval_unavailable`); `decline`, `cancel` и ответ неверной формы — тоже отказы. Read никогда не спрашивается.

Подтверждение — UX-механизм, не граница безопасности (FR-44a): ответ приходит от клиента, клиент может сформировать его сам, и отличить это невозможно. Оно способно только отнять то, что policy уже разрешила.

...
- [ ] T401 config: `inboundAuth.oauth` with `issuer`, `resource`, `audience`, `requiredScopes`,
...
- [ ] T404 inbound: the `auth.TokenVerifier` for JWTs — issuer, `exp`/`nbf`, audience/resource
...
- [ ] T405 signature: verification against a key selected by `kid` from the JWKS
- [ ] T406 scopes (FR-84): a token missing a required scope is 403, and the challenge names the
...
- [ ] T407 [P] the refusal table: wrong issuer, wrong audience, expired, not-yet-valid, unknown
...
- [ ] T408 jwks: fetch, parse (RSA and EC), cache with a TTL, select by `kid`
...
- [ ] T409 rotation: an unknown `kid` triggers at most one refetch per cooldown window
...
- [ ] T410 [P] fail closed: a fetch that fails leaves the previous key set in force; with no
